...
export class ArchService {
  ...
  pacmanGeneralSearch(query: string) {
    ...
    this.httpClient
      .get<ArchPackageQueryResponse>(`${environment.apiUrl}/arch/pacman/search?query=${query}`)
  ...
  pacmanSpecificPackage(query: string) {
    ...
    this.httpClient
      .get<ArchPackage>(`${environment.apiUrl}/arch/pacman/package/${query}`)
      .pipe(finalize(() => this.loading.set(false)))
      .subscribe({
  ...
  aurGeneralSearch(query: string) {
    ...
    this.httpClient
      .get<AurQueryResponse>(`${environment.apiUrl}/arch/aur/search?query=${query}`)
      .pipe(finalize(() => this.loading.set(false)))
      .subscribe({
  ...
  aurSpecificPackage(query: string) {
    ...
    this.httpClient
      .get<AurPackage>(`${environment.apiUrl}/arch/aur/package/${query}`)
      .pipe(finalize(() => this.loading.set(false)))
      .subscribe({
