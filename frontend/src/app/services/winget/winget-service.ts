...
export class WingetService {
  ...
  wingetGeneralSearch(query: string) {
    ...
    this.httpClient.get<WingetQueryResponse>(`http://localhost:3000/api/winget/search?query=${query}`)
      .pipe(
  ...
  wingetSpecificSearch(query: string) {
    ...
    this.httpClient.get<WingetPackage>(`http://localhost:3000/api/winget/package/${query}`)
      .pipe(
        finalize(() => this.loading.set(false))
      )
      .subscribe({
